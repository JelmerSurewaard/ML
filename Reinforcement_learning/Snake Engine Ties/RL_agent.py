...
class RL_Agent:
    ...
    def train_existing_model(self, env, timesteps, old_model_id):
        custom_objects = {"exploration_initial_eps": 1.0, "exploration_final_eps": 0.05, "exploration_fraction": 0.1}
        self.model = DQN.load("learned_models/" + str(old_model_id), env=env, custom_objects=custom_objects, verbose=0)       
        self.model.learn(timesteps)
        self.model.save("learned_models/" + str(self.id))
    ...
    def train(self, env, timesteps, final_expo_rate):
        ...
        self.model  = DQN('MlpPolicy',  env, verbose=1, learning_rate=0.005, tensorboard_log="tensorboard_logs/" + str(self.id), exploration_final_eps=final_expo_rate)
        self.model.learn(total_timesteps=timesteps)
        self.model.save("learned_models/" + str(self.id))
        
    def load(self):
        self.model = DQN.load("learned_models/" + str(self.id))
